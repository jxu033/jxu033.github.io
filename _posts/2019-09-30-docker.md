...
注意：用docker-compose启动的容器的名字会以目录的名字为前缀，对于上面的docker-compose文件的例子，启动后会有容器名：
<目录名>_db_1, <目录名>_wordpress_1. 注意到这里的"_1", 将docker-compose文件的端口映射改为随机端口映射(因为80已经被占用了)，可以通过命令`docker-compose scale wordpress=3` 
